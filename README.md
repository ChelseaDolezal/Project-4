...
1. K-Means was chosen to group the customers based on ease of implementation and effectiveness of segmentation tasks. 
We also tried the Birch Means method but it didn't give us anything different so we chose to continue with the K-Means model.

2. We used the elbow method to determine the optimal number of clusters for our K-Means model.  We settled on 4 clusters after reviewing the elbow graph and the K-Means cluster graphs.  With 4 clusters we were better able to segment the outliers.

3. Once we identified the clusters we transformed the scaled data back to the original values to analyze the different segments and provide Cluster Profiles based on RFM average values.

## Cluster Interpretation and Profiles:

### Cluster Profiles (based on average of original data values)
...
## How CLV Prediction Can Be Used
 ...
 - **Customer Segmentation:**
    - **Segment by CLV:** Beyond RFM segmentation, customers can be segmented by predicted CLV to create even more focused marketing and sales strategies.</br>
...
## Limitations 
1. **Outliers:** We achieved 80% accuracy with the model, but this required excluding the top 20% of spenders, who are likely wholesalers. 
2. **Additionally,:: it was challenging to clearly present each individual's spending habits. We had to work with a smaller dataset (Clusters) to ensure that the graphs remained clear and informative.
3. **Unaccounted external factors:** This does not account for outside factors and assumes the market will stay stable. (e.g., economic conditions, seasonal trends) which could influence spending patterns and impact the model's performance
